  ...
  return (
    <Card sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <CardContent sx={{ padding: 1 }}>
        ...
        <Typography align="center" color="textPrimary" variant="body1" sx={{ marginBottom: '8.0px' }}>
          {props.new.Summary}
        </Typography>
      </CardContent>
      ...
      <Box sx={{ p: 2 }}>
        <Grid container spacing={2} sx={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <Grid item sx={{ alignItems: 'center', display: 'flex' }}>
            <Button onClick={props.onClick} id={props.id} variant="outlined" color='success'>Ver</Button>
          </Grid>
        </Grid>
      </Box>
    </Card>
